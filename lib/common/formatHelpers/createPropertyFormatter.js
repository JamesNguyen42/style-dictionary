...
/**
 * Sass asset variables need string literals for values such as raw paths and base64 data.
 * Preserve values that are already valid Sass expressions.
 * @param {unknown} value
 * @returns {unknown}
 */
function formatSassAsset(value) {
...
export default function createPropertyFormatter({
  ...
  return function (token) {
    ...
    const tokenType = usesDtcg ? token.$type : token.type;
    if (format === sass && tokenType === 'asset') {
      value = formatSassAsset(value);
    }
