...
export default function formattedVariables({
  ...
  const outputReferencesIsFunction = typeof outputReferences === 'function';
  // Let custom predicates decide whether a filtered reference will be emitted;
  // sorting should not report it before that predicate runs.
  const byReference = outputReferences
    ? sortByReference(
        outputReferencesIsFunction ? (dictionary.unfilteredTokens ?? tokens) : tokens,
        {
          unfilteredTokens: outputReferencesIsFunction ? undefined : dictionary.unfilteredTokens,
          usesDtcg,
        },
      )
    : null;
