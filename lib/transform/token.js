...
import { StopTransformError } from '../utils/StopTransformError.js';
...
const STOP_TRANSFORMS = Symbol('stop transforms');
...
async function _transformTokenWrapper(transform, token, config, options, vol) {
  ...
  try {
    ...
    if (e instanceof StopTransformError) {
      return STOP_TRANSFORMS;
    }
  ...
  for (let i = 0; i < transforms.length; i++) {
    ...
    if (!transform.filter || transform.filter(to_ret, options)) {
      if (transform.type === transformTypeName) {
        const transformedName = await _transformTokenWrapper(
          transform,
          to_ret,
          config,
          options,
        ...
        if (transformedName === STOP_TRANSFORMS) {
          break;
        }
        to_ret.name = /** @type {string} */ (transformedName);
      }
      ...
      if (
        ...
        if (
          ...
          if (transformedValue === STOP_TRANSFORMS) {
            break;
          }
      ...
      if (transform.type === transformTypeAttribute) {
        const transformedAttributes = await _transformTokenWrapper(
          transform,
          to_ret,
          config,
          options,
        ...
        if (transformedAttributes === STOP_TRANSFORMS) {
          break;
        }
        const attrs = /** @type {Record<string, unknown>} */ (transformedAttributes);
