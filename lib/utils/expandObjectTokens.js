...
export function expandTokenInMap(token, tokenMap, opts, platform) {
  ...
  _value.forEach((objectVal, index, arr) => {
    ...
    if (!isPlainObject(objectVal)) {
      const innerKey = /** @type {string} */ (token.key).replace('}', `.${index + 1}}`);
      const expandedValue = {
        ...copyMeta,
        [`${uses$ ? '$' : ''}value`]: objectVal,
        [`${uses$ ? '$' : ''}type`]: compositionType,
        key: innerKey,
      };
    ...
    Object.entries(objectVal).forEach(([propKey, value]) => {
      ...
      // If we're expanding on platform level, we already have a path property. Adjust it before
      // recursing so nested object and array values retain their full path.
      if (Array.isArray(expandedValue.path)) {
        if (arr.length > 1) {
          expandedValue.path = [...expandedValue.path, `${index + 1}`];
        }
      ...
      // If the value is an object or array, we're dealing with a nested composite token and we have
      // to recurse.
...
export function expandTokens(tokenMap, opts, platform) {
  ...
  Array.from(copyResolved).forEach(([, token]) => {
    ...
    if (
      isPlainObject(value) ||
      // support array values, including multi-value arrays where each item is an object
